...
class ModelTrainer:
    ...
    def train_wait_time_model(self, data: dict):
        """
        Train wait time prediction model with Weather Awareness
        """
        ...
        time_col = (
            "log_timestamp" if "log_timestamp" in df.columns else "timestamp_quoted"
        )
        df[time_col] = pd.to_datetime(df[time_col])

        df["hour_of_day"] = df[time_col].dt.hour
        df["day_of_week"] = df[time_col].dt.dayofweek
        df["month"] = df[time_col].dt.month  # <--- Seasonality

        df["is_weekend"] = df["day_of_week"].isin([5, 6]).astype(int)
        df["is_peak_hour"] = (
            df["hour_of_day"].isin([11, 12, 13, 17, 18, 19, 20]).astype(int)
        )
        df["is_lunch"] = df["hour_of_day"].isin([11, 12, 13]).astype(int)
        df["is_dinner"] = df["hour_of_day"].isin([17, 18, 19, 20]).astype(int)

        # --- FIX 2: Safe Weather Encoding ---
        if "weather_condition" not in df.columns:
            df["weather_condition"] = "sunny"  # Default fallback

        # One-Hot Encode
        weather_dummies = pd.get_dummies(df["weather_condition"], prefix="weather")
        df = pd.concat([df, weather_dummies], axis=1)

        # FORCE all expected columns to exist (prevents crash if no snow in data)
        expected_weather = [
            "weather_sunny",
            "weather_rainy",
            "weather_cloudy",
        ...
        for col in expected_weather:
            if col not in df.columns:
                df[col] = 0  # Fill missing weather types with 0
        ...
        # Select features (Matches your list exactly)
        feature_columns = [
            "party_size",
            "hour_of_day",
            "day_of_week",
        ...
        logger.info(f"Model MAE: {test_mae:.2f} minutes")
    ...
    def train_busyness_model(self, data: dict):
        ...
        hourly_data = (
            orders.groupby(["date", "hour", "day_of_week"])
            .agg({"order_id": "count", "party_size": "mean", "order_total": "sum"})
            .reset_index()
        )
        ...
        hourly_data["busyness_level"] = hourly_data["order_count"].apply(
            classify_busyness
        )
        ...
        hourly_data["is_dinner"] = (
            hourly_data["hour"].isin([17, 18, 19, 20]).astype(int)
        )
        hourly_data["is_peak_hour"] = (
            hourly_data["hour"].isin([11, 12, 13, 17, 18, 19, 20]).astype(int)
        )
        ...
        hourly_data = hourly_data.merge(
            external[["date", "is_holiday", "weather_condition"]], on="date", how="left"
        )
        ...
        if "weather_condition" in hourly_data.columns:
            weather_dummies = pd.get_dummies(
                hourly_data["weather_condition"], prefix="weather"
            )
        ...
        for level in ["slow", "moderate", "peak"]:
            if level in report:
                logger.info(
                    f"   {level.capitalize()}: Precision={report[level]['precision']:.3f}, Recall={report[level]['recall']:.3f}"
                )
        ...
        self.training_report["feature_importance"]["busyness"] = (
    ...
    def train_item_sales_model(self, data: dict):
        ...
        order_items = order_items.merge(
            menu[["item_name", "category"]], on="item_name", how="left"
        )

        # Aggregate by item and date
        daily_sales = (
            order_items.groupby(["item_name", "category", "date", "day_of_week"])
            .agg({"quantity": "sum"})
            .reset_index()
        )
        ...
        daily_sales["historical_avg_sales"] = daily_sales["item_name"].map(
            item_avg_sales
        )
        ...
        top_items = (
            order_items.groupby("item_name")["quantity"].sum().nlargest(10).index
        )
        ...
        # Select features
        category_features = [
            col for col in daily_sales_top.columns if col.startswith("category_")
        ]
        feature_columns = [
            "day_of_week",
            "month",
        ...
        self.training_report["feature_importance"]["item_sales"] = (
            feature_importance.to_dict("records")
        )
