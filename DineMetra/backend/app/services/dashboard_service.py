    ...
    def get_sales_chart_data(self, period: str = "this-week") -> List[Dict]:
        ...
        try:
            ...
            try:
                ...
                sales_dict = {str(record.date): float(record.total_sales or 0) 
                ...
                # Calculate trend
                if len(chart_data) >= 2:
                    sales_values = [d['sales'] for d in chart_data if d['sales'] > 0]
                    if len(sales_values) >= 2:
                        first_avg = sum(sales_values[:3]) / 3 if len(sales_values) >= 3 else sales_values[0]
                        last_avg = sum(sales_values[-3:]) / 3 if len(sales_values) >= 3 else sales_values[-1]
                        trend = ((last_avg - first_avg) / first_avg * 100) if first_avg > 0 else 0
                    else:
                        trend = 0
                else:
                    trend = 0
