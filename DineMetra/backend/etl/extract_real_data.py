...
class RealDataExtractor:
    ...
    def _process_csv_file(self, file_path: Path):
        ...
        try:
            ...
            if 'yyyymmdd' in columns or ('net sales' in columns and len(df) > 20):
                ...
                logger.info(f"   ✓ Loaded hourly pattern ({len(self.hourly_pattern)} hours)")
        ...
        for _, day_row in self.daily_sales.iterrows():
            ...
            for hour, hour_pct in hourly_distribution.items():
                ...
                for _ in range(orders_this_hour):
                    ...
                    order_total = self._generate_order_total(party_size, daily_revenue / daily_orders)
                    ...
                    tip_amount = subtotal * random.uniform(0.15, 0.22) if order_type == 'dine_in' else 0
                    
                    # Duration (prep time)
                    duration_minutes = int(random.gauss(45, 15))  # Average 45 min, std 15 min
                    ...
                    orders.append({
                        ...
                        'table_number': f"T{random.randint(1, 30)}" if order_type == 'dine_in' else None,
                        'server_id': random.randint(100, 110) if order_type == 'dine_in' else None,
    ...
    def _generate_party_size(self) -> int:
        ...
        return np.random.choice(
            [1, 2, 3, 4, 5, 6, 7, 8],
            p=[0.12, 0.38, 0.18, 0.20, 0.06, 0.03, 0.02, 0.01]
        )
    ...
    def generate_wait_times(self, orders_df: pd.DataFrame) -> pd.DataFrame:
        ...
        hourly_counts = orders_df.groupby(['date', 'hour']).size().reset_index(name='order_count')
    ...
    def _generate_summary_report(self, orders_df: pd.DataFrame, 
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"   Date Range: {orders_df['order_timestamp'].min().date()} to {orders_df['order_timestamp'].max().date()}\n")
            ...
            f.write(f"   Avg Quoted Wait: {wait_times_df['quoted_wait_minutes'].mean():.1f} min\n")
            f.write(f"   Avg Actual Wait: {wait_times_df['actual_wait_minutes'].mean():.1f} min\n")
            f.write(f"   Avg Accuracy: {((wait_times_df['quoted_wait_minutes'] - wait_times_df['actual_wait_minutes']).abs().mean()):.1f} min error\n\n")
            
            f.write("📅 BUSIEST DAYS:\n")
            daily = orders_df.groupby(pd.to_datetime(orders_df['order_timestamp']).dt.date).agg({
                'order_id': 'count',
                'order_total': 'sum'
            }).nlargest(5, 'order_id')
            
            for date, row in daily.iterrows():
                f.write(f"   {date}: {row['order_id']} orders, ${row['order_total']:.2f}\n")
