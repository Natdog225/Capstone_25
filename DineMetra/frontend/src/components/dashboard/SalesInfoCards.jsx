...
const SalesInfoCards = ({ dateRange }) => {
  ...
  useEffect(() => {
    const fetchInfoSections = async () => {
      try {
        ...
        console.log('SalesInfoCards - Raw API data:', data);
        ...
        if (data?.events) {
          ...
          console.log(`SalesInfoCards - Filtered out ${data.events.length - filteredEvents.length} events`);
